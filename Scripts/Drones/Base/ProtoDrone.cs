namespace AtomicTorch.CBND.CoreMod.Drones
{
      ...
      where TClientState : BaseClientState, new()
  {
    ...
    protected override void ServerUpdate(ServerUpdateData data)
    {
      ...
      if (hasMiningTargets
          && !(CharacterDroneControlSystem.SharedIsValidDroneOperationDistance(objectDrone.TilePosition, characterOwner.TilePosition)
               )) //&& objectDrone.Tile.Height == characterOwner.Tile.Height))
      {
        Logger.Info("The drone is beyond operation distance and will be recalled: "
                    + objectDrone);
