  ...
  public class StatusEffectCold : ProtoStatusEffect
  {
    ...
    protected override void ServerUpdate(StatusEffectData data)
    {
      ...
      if (data.Intensity >= 1.0)
      {
        ...
        double intensity = data.DeltaTime * (1.0 / StatusEffectFrostbite.EffectAccumulationDuration);

        // modify intensity based on effect multiplier
        intensity *= data.Character.SharedGetFinalStatMultiplier(StatName.ColdEffectMultiplier);
    ...
    private void ServerUpdateStatusEffectIntensity(ICharacter character, double deltaTime)
    {
      ...
      if (environmentalColdIntensity > 0)
      {
        ...
        // modify intensity based on effect multiplier
        delta *= character.SharedGetFinalStatMultiplier(StatName.ColdEffectMultiplier);
      }
      else
      {
