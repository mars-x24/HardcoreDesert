...
namespace AtomicTorch.CBND.CoreMod.Helpers.Client.Physics
{
  ...
  public class ClientComponentPhysicsSpaceVisualizer : ClientComponent
  {
    ...
    public static readonly Brush BrushWaterCollider = new SolidColorBrush(Color.FromArgb(0xCC, 0x73, 0xB2, 0xFA)); //mod

    public static readonly Brush BrushHoverWaterCollider = new SolidColorBrush(Color.FromArgb(0xCC, 0x56, 0x3F, 0x8A)); //mod
      ...
      this.legendLayers = new List<ViewModelPhysicsGroup>()
            {
                ...
                new(
                    "Water colliders",
                    CollisionGroupId.Water,
                    EnabledLayers.Contains(CollisionGroupId.Water),
                    BrushWaterCollider), //mod
                new(
                    "Hover Water colliders",
                    CollisionGroupId.HoverWater,
                    EnabledLayers.Contains(CollisionGroupId.HoverWater),
                    BrushHoverWaterCollider) //mod
            };
    ...
    private bool GetBrushByCollisionGroup(IPhysicsShape shape, out Brush brush)
    {
      ...
      if (shape.CollisionGroup == CollisionGroups.Water) //mod
      {
        brush = BrushWaterCollider;
        return true;
      }

      if (shape.CollisionGroup == CollisionGroups.HoverWater) //mod
      {
        brush = BrushHoverWaterCollider;
        return true;
      }
