...
using static AtomicTorch.CBND.CoreMod.Tiles.TileBlendSides;
  ...
  public static class ClientTileBlendHelper
  {
        ...
        bool isHeightsBlendPhase)
    {
      ...
      foreach (var blendLayer in blendLayers)
      {
        ...
        {
          //MOD
          if ((tileProto.Kind == TileKind.Solid && blendWith.ProtoTile.Kind == TileKind.Water) ||
            (tileProto.Kind == TileKind.Water && blendWith.ProtoTile.Kind == TileKind.Water && tileProto.BlendOrder != blendWith.ProtoTile.BlendOrder))
          {
            // create water blend
