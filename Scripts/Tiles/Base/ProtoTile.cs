  ...
  public abstract class ProtoTile : ProtoEntity, IProtoTile
  {
    ...
    public static double SharedGetTileMoveSpeedMultiplier(in Tile tile)
    {
      ...
      foreach (var staticWorldObject in tile.StaticObjects)
      {
        if (staticWorldObject.ProtoGameObject is IProtoObjectMovementSurface protoObjectMovementSurface
            && (moveSpeedMultiplier < protoObjectMovementSurface.CharacterMoveSpeedMultiplier || tile.ProtoTile.Kind == TileKind.Water)) //mod
        {
    ...
    public virtual void CreatePhysics(Tile tile, IPhysicsBody physicsBody)
    {
      ...
      if (tileKind == TileKind.Water)
      {
        ...
        // mod HoverBlocking near limits
        if (this.IsHoverBlocking(tile, out _))
        ...
        foreach (var neighborTile in tile.EightNeighborTiles)
        {
          ...
          if (!Rates.RateHoverWaterEnabled.SharedValue)
            physicsBody.AddShapeRectangle((1, 1), null, CollisionGroups.HoverWater);
    ...
    public bool IsHoverBlocking(Tile tile, out bool isCliff)
    {
      //mod
      byte limit = Rates.RateBlockWaterTileNumber.SharedValue;

      ushort offX = (ushort)((IsServer ? Server.World.WorldBounds.Offset.X : Client.World.WorldBounds.Offset.X) - 1);
      ushort offY = (ushort)((IsServer ? Server.World.WorldBounds.Offset.Y : Client.World.WorldBounds.Offset.Y) - 1);
      ushort maxX = IsServer ? Server.World.WorldBounds.MaxX : Client.World.WorldBounds.MaxX;
      ushort maxY = IsServer ? Server.World.WorldBounds.MaxY : Client.World.WorldBounds.MaxY;

      isCliff = (tile.Position.X - offX) == limit ||
                (tile.Position.Y - offY) == limit ||
                (maxX - tile.Position.X) == limit ||
                (maxY - tile.Position.Y) == limit;

      return (tile.Position.X - offX) <= limit ||
             (tile.Position.Y - offY) <= limit ||
             (maxX - tile.Position.X) <= limit ||
             (maxY - tile.Position.Y) <= limit;
    }
    ...
    public void ServerRefresh(Tile tile)
    {
      ...
      if (!Api.IsEditor)
      {
        if (tile.ProtoTile is ProtoTileWater && !(tile.ProtoTile is TileDarkIce) && this.IsHoverBlocking(tile, out bool isCliff))
        {
          Server.World.SetTileData(tile.Position, Api.GetProtoEntity<TileDarkIce>(), tile.Height, false, false); //(byte)(tile.Height + 16), false, false);
          return;
        }
      }
