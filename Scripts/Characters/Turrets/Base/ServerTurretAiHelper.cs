  ...
  public static class ServerTurretAiHelper
  {
        ...
        bool forceAttackNeutrals)
    {
      ...
      if (worldObject is not ICharacter targetCharacter
          || (targetCharacter.IsNpc && targetCharacter.ProtoGameObject is not IProtoCharacterMob))
      {
        return false;
      }
        ...
        CharacterMobPrivateState turretPrivateState)
    {
      ...
      static bool IsValidTargetCallback(IWorldObject worldObject)
          => worldObject is ICharacter { IsNpc: false }
             || worldObject.ProtoGameObject is IProtoVehicle
             || worldObject.ProtoGameObject is IProtoCharacterMob;
    }
        ...
        bool forceAttackNeutrals)
    {
      ...
      if (targetCharacter.ProtoGameObject is IProtoCharacterMob)
      {
        bool aggro = false;

        var privateStateMobEnraged = targetCharacter.GetPrivateState<CharacterMobEnragedPrivateState>();
        if (privateStateMobEnraged is not null)
          aggro = privateStateMobEnraged.CurrentAggroCharacter is not null || privateStateMobEnraged.CurrentTargetCharacter is not null || privateStateMobEnraged.CurrentTargetStructure is not null;
        else
        {
          var privateStateMob = targetCharacter.GetPrivateState<CharacterMobPrivateState>();
          aggro = privateStateMob.CurrentAggroCharacter is not null || privateStateMob.CurrentTargetCharacter is not null;
        }
