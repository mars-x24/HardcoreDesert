...
namespace AtomicTorch.CBND.CoreMod.Characters.Player
{
                ...
                PlayerCharacterClientState>
  {
    ...
    protected override void ServerUpdate(ServerUpdateData data)
    {
      ...
      if (character.Tile.ProtoTile.Kind == TileKind.Water)// && !Api.IsEditor)
      {
        //No vehicle
        if (publicState.CurrentVehicle is null)
        {
          //No platform
          if (character.Tile.StaticObjects.All(o => o.ProtoStaticWorldObject.Kind != StaticObjectKind.Platform))
          {
            //Move player to shore
            if (!ServerMovePlayerToShore(character))
            {
    ...
    public static bool ServerMovePlayerToShore(IDynamicWorldObject worldObject)
    {
      var worldBounds = Server.World.WorldBounds;
      var offset = worldBounds.Offset;
      var size = worldBounds.Size;
      var centerPosition = new Vector2F(offset.X + size.X / 2,
                                        offset.Y + size.Y / 2);

      var v = centerPosition - worldObject.Position.ToVector2F();

      //nearest tile short distance
      var t1 = GetNearestShoreTile(worldObject, v, 1, 5, 1);
      if (t1.HasValue && ServerTryPlaceCharacterNearby(worldObject, t1.Value.Position.ToVector2D()))
        return true;

      //nearest tile mid distance
      var t2 = GetNearestShoreTile(worldObject, v, 5, 20, 2);
      if (t2.HasValue && ServerTryPlaceCharacterNearby(worldObject, t2.Value.Position.ToVector2D()))
        return true;

      //nearest tile long distance
      var t3 = GetNearestShoreTile(worldObject, v, 20, 80, 5);
      if (t3.HasValue && ServerTryPlaceCharacterNearby(worldObject, t3.Value.Position.ToVector2D()))
        return true;

      //nearest tile very long distance
      var t4 = GetNearestShoreTile(worldObject, v, 80, 500, 20);
      if (t4.HasValue && ServerTryPlaceCharacterNearby(worldObject, t4.Value.Position.ToVector2D()))
        return true;

      //nearest tile mega long distance
      var t5 = GetNearestShoreTile(worldObject, v, 500, 1500, 100);
      if (t5.HasValue && ServerTryPlaceCharacterNearby(worldObject, t5.Value.Position.ToVector2D()))
        return true;

      return false;
    }

    private static Tile? GetNearestShoreTile(IDynamicWorldObject worldObject, Vector2F firstVector, int minDistance, int maxDistanceBeforeRotate, int increaseValue)
    {
      for (float rotateDeg = 0; rotateDeg < 360; rotateDeg += 45)
      {
        for (int i = minDistance; i < maxDistanceBeforeRotate; i += increaseValue)
        {
          var v = (firstVector.Normalized * (minDistance + i)).RotateDeg(rotateDeg);
          Tile t = worldObject.Tile.GetNeighborTile(v.ToVector2Int());
          if (t.IsValidTile && t.ProtoTile.Kind == TileKind.Solid)
            return t;
        }
      }
            ...
            Vector2D startPosition)
    {
      var radiuses = new[]
      {
          (min: 0, max: 2, attempts: 4),
          (min: 2, max: 5, attempts: 8),
          (min: 5, max: 20, attempts: 15)
      };

      var restrictedZone = ZoneServerRestricted.Instance.ServerZoneInstance;
      var physicsSpace = Server.World.GetPhysicsSpace();

      foreach (var radius in radiuses)
      {
        int attemptsRemains = radius.attempts;
        ...
        do
        {
          ...
          Server.World.SetPosition(worldObject, spawnPosition);
          return true;
        }
    ...
    protected override void SharedCreatePhysics(CreatePhysicsData data)
    {
      ...
      else // has a vehicle
      {
        ...
        if (protoVehicle is IProtoVehicleLootOnWater)
        {
          physicsBody.AddShapeCircle(
            center: (0, 0.65),
            radius: characterInteractionRadius,
