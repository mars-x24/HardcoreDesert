...
namespace AtomicTorch.CBND.CoreMod.ClientComponents.StaticObjects
{
  public class ClientComponentObjectInteractionHelper : ClientComponent
  {
    ...
    private static void SetCurrentMouseOverObject(IWorldObject value)
    {
      ...
      //MOD - cannot interact while on vehicle
      var currentVehicle = ClientCurrentCharacterHelper.Character.GetPublicState<PlayerCharacterPublicState>().CurrentVehicle;
      if (currentVehicle is not null && !(currentVehicle.ProtoGameObject is IProtoVehicleLootOnWater && currentVehicle.Tile.ProtoTile.Kind == TileKind.Water))
      {
        ClientCursorSystem.CurrentCursorId = CursorId.Default;
        InteractionTooltip.Hide();
        WorldObjectTitleTooltip.Hide();
    ...
    private static void Update()
    {
      ...
      // MOD - cannot interact while on vehicle
      var currentVehicle = character.GetPublicState<PlayerCharacterPublicState>().CurrentVehicle;
      if (currentVehicle is not null && !(currentVehicle.ProtoGameObject is IProtoVehicleLootOnWater && currentVehicle.Tile.ProtoTile.Kind == TileKind.Water))
      {
        InteractingWithObject = null;
        return;
